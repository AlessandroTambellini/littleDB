...
typedef struct {
	uint32_t id;
	char username[COL_USERNAME_SIZE + 1]; // +1 is for '\0'
...
const uint32_t INTERNAL_NODE_MAX_KEYS = 3; // Kept small for testing
...
typedef struct {
	...
	uint32_t file_size;
	uint32_t num_pages;
	void *pages[TABLE_MAX_PAGES]; // initialized to NULL by default (at least in C11)
} Pager;
...
/*
	...
	When I need a function, I declare it. 
	So, functions are ordered based on the time of creation.
*/
...
MetaCmdRes do_meta_cmd(InputBuffer *input_buffer, Table *table);
void close_input_buffer(InputBuffer *input_buffer);
void db_close(Table *table);
void pager_flush(Pager *pager, uint32_t page_num);
void print_constants();
void print_leaf_node(void *node); // for debugging
uint32_t *leaf_node_key(void *node, uint32_t cell_num);
void printHelpSection();

StmtRes set_stmt(InputBuffer *input_buffer, Stmt *stmt);
StmtRes set_insert(InputBuffer *input_buffer, Stmt *stmt);

ExecRes exec_stmt(Stmt *stmt, Table *table);
ExecRes exec_insert(Stmt *stmt, Table *table);
...
ExecRes exec_select(Table *table);
...
Pager *pager_open(const char *filename) {
	ssize_t fd = open(filename, 
					O_RDWR | O_CREAT,  // read, write, create 
					S_IWUSR | S_IRUSR);	// read-write user permissions
	if (fd == -1) {
		perror("Unable specified db file");
		exit(EXIT_FAILURE);
	}
	...
	if (file_size % PAGE_SIZE != 0) {
		fprintf(stderr, "db file is not a whole number of pages. Corrupted file.\n");
		exit(EXIT_FAILURE);
	}
...
void read_input(InputBuffer *input_buffer) {
	...
	// support for both lowercase and uppercase SQL syntax
	for (size_t i = 0; i < input_buffer->buffer_len; i++) {
		input_buffer->buffer[i] = tolower(input_buffer->buffer[i]);
	}

	// Ignore trailing '\n' (new line char)
	input_buffer->buffer[bytes_read - 1] = 0;
	input_buffer->buffer_len = bytes_read - 1;
}
...
MetaCmdRes do_meta_cmd(InputBuffer *input_buffer, Table *table) {
	if (strcmp(input_buffer->buffer, ".exit") == 0) {
		...
		db_close(table);
		exit(EXIT_SUCCESS);
	} else if (strcmp(input_buffer->buffer, ".consts") == 0) {
		printf("Constants:\n");
		print_constants();
...
void close_input_buffer(InputBuffer *input_buffer) {
	// order of freed resources matters
...
/*
	tasks:
		...
		- close db file
		- free mem for pager ds and table ds
*/
void db_close(Table *table) {
	...
	ssize_t res = close(pager->fd);
	if (res == -1) {
		perror("Error closing db file");
		exit(EXIT_FAILURE);
	}

	// why this. wasn't it done above?
...
void pager_flush(Pager *pager, uint32_t page_num) {
	...
	if (offset == -1) {
		perror("Error getting bytes offset of page");
		exit(EXIT_FAILURE);
	}
...
void printHelpSection() {
	printf("                                ______   _____              \n");
	printf(" _   __   __     __    _       |  ___ \\ |  __  \\              \n");
	printf("| |  |_| _| |_  _| |_ | |  ___ | |   | || |__| |             \n");
	printf("| |  __ |_   _||_   _|| | /___\\| |   | ||  __  \\              \n");
	printf("| |  | |  | |    | |  | | |    | |___| || |__| |                   \n");
	printf("\\_\\  |_|  \\_\\    \\_\\  \\_\\ \\___/|______/ |______/        \n");
	printf("\n");
	printf("- meta commands:\n");
	printf("\t- .exit: exit the program\n");
	printf("\t- .consts: print the constants\n");
	printf("\t- .btree: print the b-tree\n");
	printf("\n");
	printf("- commands:\n");
	printf("\t- select: select from db\n");
	printf("\t- insert <id> <name> <email>: insert into db\n");
}

StmtRes set_stmt(InputBuffer *input_buffer, Stmt *stmt) {
	if (strncmp(input_buffer->buffer, "insert", strlen("insert")) == 0) {
		return set_insert(input_buffer, stmt);
	} else if (strcmp(input_buffer->buffer, "select") == 0) {
		stmt->type = STMT_SELECT;
		return STMT_SUCCESS;
	}
	return UNRECOGNIZED_STMT;
}
...
StmtRes set_insert(InputBuffer *input_buffer, Stmt *stmt) {
	...
	strtok(input_buffer->buffer, " "); // char *keyword
	char *id_string = strtok(NULL, " ");
	char *username = strtok(NULL, " ");
	char *email = strtok(NULL, " ");
	if (id_string == NULL || username == NULL || email == NULL) {
		return STMT_SYNTAX_ERR;
	}
	errno = 0; // reset error flag
	char *endptr = NULL;
	int id = strtol(id_string, &endptr, 10);
	if (errno != 0 || *endptr != '\0' || id < 0) {
		return STMT_INVALID_ID;
	} else if (strlen(username) > COL_USERNAME_SIZE) {
		return STMT_STR_TOO_LONG;
	} else if (strlen(email) > COL_EMAIL_SIZE) {
		return STMT_STR_TOO_LONG;
	} 
...
/*
	This will either return:
		...
		- the position of another key 
			that we’ll need to move if we want to insert the new key
...
Cursor *internal_node_find(Table *table, uint32_t page_num, uint32_t key) {
	...
	switch (get_node_type(child)) {
		...
		default:
			fprintf(stderr, "Warning: node type not recognized.");
			return NULL;
	}
}
...
uint32_t *internal_node_child(void *node, uint32_t child_num) {
	...
	if (child_num > num_keys) {
		...
		if (*child == INVALID_PAGE_NUM) {
			fprintf(stderr, "Tried to access child %d of node, but was invalid page\n", child_num);
			exit(EXIT_FAILURE);
		}
		return child;
	}
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value) {
	...
	*/
	for (ssize_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
...
void create_new_root(Table *table, uint32_t right_child_page_num) {
	...
	if (get_node_type(left_child) == NODE_INTERNAL) {
		void *child;
		for (size_t i = 0; i < *internal_node_num_keys(left_child); i++) {
...
void internal_node_split_and_insert(Table *table, uint32_t parent_page_num,
	...
	*internal_node_right_child(old_node) = INVALID_PAGE_NUM;
	...
	for (size_t i = INTERNAL_NODE_MAX_KEYS - 1; i > INTERNAL_NODE_MAX_KEYS / 2; i--) {
	...
	// Set child before middle key, 
	// which is now the highest key, to be node's right child,
...
// get and store data into a page ds
void deserialize_row(void *src, Row *dest) {
...
int main(int argc, const char *argv[]) {
	...
	while (true) {
		...
		/*
			I do not like to use "continue" if there are both switch() and while().
			It creates confusion in my head. So, I use conditionals with flags,
			in this case execStmt.
		*/
		if (input_buffer->buffer[0] == '.') {
			...
			switch (set_stmt(input_buffer, &stmt)) {
				case STMT_SUCCESS:
					execStmt = true;
					break;
				case STMT_INVALID_ID:
					fprintf(stderr, "Invalid ID.\n");
					break;
				case STMT_STR_TOO_LONG:
					fprintf(stderr, "The length of a field is too long.\n");
					break;
				case STMT_SYNTAX_ERR:
					fprintf(stderr, "Syntax error: could not parse statement.\n");
					break;
				case UNRECOGNIZED_STMT:
		...
		if (execStmt) {
			switch (exec_stmt(&stmt, table)) {
				...
				case EXEC_DUPLICATE_KEY:
					...
					break;
				case EXEC_ERR:
					fprintf(stderr, "Error: statement %d not supported.", stmt.type);
			}
		}
	}
}
